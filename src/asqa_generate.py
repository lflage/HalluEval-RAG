...
parser = argparse.ArgumentParser(description="CLI for configuring the setups for ASQA experiments.")

parser.add_argument("--model-name", type=str, default="meta-llama/Llama-3.1-8B-Instruct",
                    help="Model name or path")
parser.add_argument("--out-dir", type=str, default="./results/asqa",
                    help="Output directory for results")
parser.add_argument("--dataset-path", type=str, default="./data/datasets/asqa_contexts.jsonl",
...
OUT_FILE = f"asqa_{model_base_name}_is_rag={IS_RAG}_is_summ={IS_SUMM}.jsonl"
OUT_PATH = os.path.join(OUT_DIR, OUT_FILE)

with open (DATASET_PATH, "r", encoding="utf8") as f:
    asqa = [json.loads(line) for line in f]
...
for sample in tqdm(asqa):
    ...
    if use_chat:
        ...
        question = tokenizer.apply_chat_template(instruct_dict, tokenize=False, add_generation_prompt=True)
    ...
    sample_out = {
        "topic"     :   [page["title"] for page in sample["wikipages"]],
        "input"     :   sample['ambiguous_question'],
        "prompt"    :   question,
        "rag"       :   IS_RAG,
        "output"    :   out_text,
    }
